...
class Particle {
  ...
  kill(width: number, height: number) {
    if (!this.isKilled) {
      ...
      // Begin blending color to black for fade out
      this.startColor = {
...
const DEFAULT_WORDS = ["YEOUBI", "YEOUBI"]
...
export function ParticleTextEffect({ 
  ...
  const nextWord = (word: string, index: number, canvas: HTMLCanvasElement) => {
    ...
    // Consistent grayscale light color scheme to guarantee strict black & white
    const grayValue = 240 + Math.floor(Math.random() * 15) // Beautiful silver whitespace
    const newColor = {
      r: grayValue,
      g: grayValue,
      b: grayValue,
    ...
    // Dynamic steps to keep overall particle count highly optimized ~1000 - 1500
    // Higher resolution screen -> larger font height -> higher step size.
    const pixelSteps = Math.max(3, Math.ceil(fontHeight / 19))
    ...
    // Shuffle coordinates for fluid movement
    ...
    for (const coordIndex of coordsIndexes) {
      ...
      if (alpha > 0 && luminosity > 50) {
        ...
        if (particleIndex < particles.length) {
          ...
          particle.maxSpeed = Math.random() * 7 + 4
  ...
  const animate = () => {
    ...
    // Background with motion blur to render trails
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      nextWord(words[wordIndexRef.current], wordIndexRef.current, canvas)
    }
